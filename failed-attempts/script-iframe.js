...
function injectTooltipHTML(num, css, desc, stepUrl){
    ...
    div.style.cssText = css + ' border: none; background-color: transparent';
    div.id = divID;
    ...
    divClosed.addEventListener('mousedown', function(e){
        ...
        if(e.button == 0){
            var iframe = document.getElementById('HelpCall_'+n);
            iframe.contentWindow.document.getElementById('HelpCall-ttB_'+n).style.display = 'block';
        }
    });
    ...
    div.addEventListener("load", function(){
        var iframeDoc = div.contentDocument || div.contentWindow.document;
        ...
        iframeDoc.body.appendChild(divClosed);
        iframeDoc.body.appendChild(divOpen);
        div.style.height = '60px';      // doesn't work when the text gets longer
        div.style.width = "180px";      // can't click beside the tooltip
        div.scrolling = "no";
    })
...
async function create_tooltip(e) {
    ...
    css = nodeToCSS(e.target, e.cloned, e.absX, absY=e.absY);        // TODO check if we need to use target or cloned!
